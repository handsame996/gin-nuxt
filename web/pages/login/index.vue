<!-- pages/index.vue -->
<template>
  <div class="body-main">
     <div class="main">
            <div :class="['container','a-container',{ 'is-txl': isHidden }]" id="a-container">
                <a-form id="a-form" class="form" method="" action=""  @submit.prevent>
                    <h2 class="form_title title">{{$t('create_account')}}</h2>
                    <div class="flex">
                        <img class="form__icon" src="data:image/svg+xml;base64," alt="">
                        <img class="form__icon" src="data:image/svg+xml;base64,">
                        <img class="form__icon" src="data:image/svg+xml;base64,">
                    </div>
                    <span class="form__span">or use email for registration</span>
                    <a-input class="form__input" type="text" placeholder="Name" />
                    <a-input class="form__input" type="text" placeholder="Email"/>
                    <a-input class="form__input" type="password" placeholder="Password" />
                    <a-button class="form__button button submit" htmlType="button">{{$t('register')}} </a-button>
                </a-form>
            </div>
            <div :class="['container','b-container', {'is-txl': isHidden, 'is-z200': isHidden }]" id="b-container">
                <a-form id="b-form" class="form" method="" action=""  @submit.prevent>
                    <h2 class="form_title title">{{$t("sign_in_to_website")}}</h2>
                    <div class="flex">
                        <img class="form__icon" src="data:image/svg+xml;base64," alt="">
                        <img class="form__icon" src="data:image/svg+xml;base64,">
                        <img class="form__icon" src="data:image/svg+xml;base64,">
                    </div>
                    <span class="form__span">or use your email account</span> 
                    <a-input class="form__input" type="text"  v-model:value="formData.user_name" placeholder="username" />
                    <a-input class="form__input" type="password" v-model:value="formData.password" placeholder="Password" />
                    <a class="form__link">Forgot your password?</a>
                    <a-button class="form__button button submit" htmlType="button" @click="loginBtn">{{$t('login')}}</a-button>
                </a-form>
            </div>
            <div :class="['switch',{ 'is-txr': isTxr, 'is-gx': isGx }]"  id="switch-cnt" >
                <div :class="['switch__circle',{ 'is-txr': isTxr }]"></div>
                <div :class="['switch__circle','switch__circle--t',{ 'is-txr': isTxr }]"></div>
                <div :class="['switch__container',{ 'is-hidden': isHidden }]" id="switch-c1">
                    <h2 class="switch__title title">{{$t('welcome')}}</h2>{{ $t("welcome_info") }}
                    <p class="switch__description description"></p>
                    <a-button class="switch__button button switch-btn" @click="changeForm">{{$t('login')}}</a-button>
                </div>
                <div :class="['switch__container',{ 'is-hidden': !isHidden }]" id="switch-c2">
                    <h2 class="switch__title title">Hello Friend !</h2>
                    <p class="switch__description description">Enter your personal details and start journey with us</p>
                    <a-button class="switch__button button switch-btn" @click="changeForm">{{$t('register')}}</a-button>
                </div>
            </div>
        </div>
  </div>
</template>

<script lang="ts" setup>
import { ref,watch } from 'vue';
import type { resType } from '~/types/api';
definePageMeta({
  layout:false,
})
// 响应式状态管理
const isTxr = ref(false);      // 控制切换方向的类
const isHidden = ref(false);   // 控制表单显示隐藏
const isGx = ref(false);       // 控制过渡动画的类

// 表单切换逻辑
const changeForm = (e: Event) => {
  e.preventDefault();
  
  // 添加过渡动画类
  isGx.value = true;
  setTimeout(() => {
    isGx.value = false;
  }, 1500);

  // 切换状态
  isTxr.value = !isTxr.value;
  isHidden.value = !isHidden.value;
};

// 阻止表单默认提交行为
const handleSubmit = (e: Event) => {
  e.preventDefault();
};


const { login } = useBaseApi()

var formData  = ref<{user_name: string; password: string;}>({
  user_name:"",password:""
})


const loginBtn = async() => {
  const result = await login(formData.value)
  if (result.data.value?.code == 0) {
    const userCookie = useCookie('token', {
    maxAge: 60 * 60 * 24 * 7, // 过期时间：7天（秒为单位）
    sameSite: 'lax' // 防止CSRF攻击
    })
  
    // 设置Cookie值（会自动序列化为JSON）
    userCookie.value = result.data.value.data.token
    navigateTo('/admin');
  }
}

// 暴露handleSubmit方法供父组件使用
defineExpose({
  handleSubmit
});
</script>

<style lang="less" scoped>
@import "@/assets/less/login.less";
</style>